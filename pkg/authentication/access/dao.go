...
func getDao(db *sql.DB, tableName string) dao {
	return dao{
		db: db,
		...
		sqlCreateTable:   fmt.Sprintf(CREATE_QL, tableName),
		sqlTruncateTable: fmt.Sprintf(TRUNCATE_QL, tableName),

		sqlTableExist: fmt.Sprintf(TABLE_EXIST_QL, tableName),

		sqlInsertRow:    fmt.Sprintf(INSERT_QL, tableName),
		sqlUpdateRow:    fmt.Sprintf(UPDATE_QL, tableName),
		sqlDeleteAll:    fmt.Sprintf(DELETE_ALL_QL, tableName),
		sqlDeleteRow:    fmt.Sprintf(DELETE_QL, tableName),
		sqlDeleteByArgs: fmt.Sprintf(DELETE_BY_ARGS, tableName),

		sqlSelectAll:   fmt.Sprintf(SELECT_ALL_QL, tableName),
		sqlSelectWhere: fmt.Sprintf(SELECT_WHERE_QL, tableName),
	}
}
...
func (d dao) DeleteByArgs(ctx context.Context, pType string, rule []string) error {
	...
	for idx, arg := range rule {
		if arg != "" {
			...
			sqlBuf.WriteString(fmt.Sprintf("=$%d", 2+idx))
